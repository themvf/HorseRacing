"""
Parx Racing Predictive Engine v3.0 (Kiro)
...
Changes from v2:
  - Fix 1: Replaced bare except blocks with specific exception types + warning logs
  - Fix 2: Replaced flawed x/max() normalization with proper min-max scaling via _minmax_scale()
  - Fix 3: Removed dead _parse_horse_line method; externalized model weights to MODEL_WEIGHTS
"""
...
class ParxRacingEngineV3:
    """
    Parx Racing Engine v3 (Kiro).
    Extracts comprehensive data and builds predictive models.

    Improvements over v2:
      - Specific exception handling with warning logs throughout parsing
      - Proper min-max normalization that handles flat distributions per-row
      - Externalized model weights (MODEL_WEIGHTS) for easy tuning
      - Removed unreachable _parse_horse_line dead code
    """

    # ---------------------------------------------------------------------------
    # Fix 3: Model weights as a class-level constant.
    ...
    # ---------------------------------------------------------------------------
    MODEL_WEIGHTS = {
        'PP_Score_Norm':     0.25,  # Prime Power rating
        'Market_Prob_Norm':  0.15,  # Implied probability from morning-line odds
        'Trainer_WPct_Norm': 0.10,  # Trainer win percentage
        'Speed_Norm':        0.20,  # Best speed figure
        'Form_Norm':         0.15,  # Recent finish positions (inverted — lower is better)
        'Style_Norm':        0.10,  # Running style number
        'WinPct_Norm':       0.05,  # Career win percentage
    }
    ...
    # ---------------------------------------------------------------------------
    # Fix 2: Min-max scaling helper.
    # Returns a per-row scaled Series in [0, 1].
    # When all values are identical (zero variance), returns 0.5 for every row
    # so the feature contributes nothing to differentiation rather than inflating
    # every horse's score equally.
    # ---------------------------------------------------------------------------
    @staticmethod
    ...
    def _parse_horse_block(self, block, name, style_str, style_num):
        ...
        # Fix 1: specific exception type + warning log
        try:
        ...
        if odds_match:
            # Fix 1: specific exception types + warning log
            try:
        ...
        # Jockey
        jky_match = re.search(
            r'([A-Z][a-z]+)\s+([A-Z][a-z\-\'\.]+?)\s+\(([\d]+)\s+([\d]+)-([\d]+)-([\d]+)\s+(\d+)%\)', block
        )
        if jky_match:
    ...
    # Fix 3: _parse_horse_line removed — it was never called and duplicated
    # logic already handled in _parse_horse_block.

    def predict_race(self, race_num, model_type="enhanced"):
        """
        Predict race outcomes using multiple features.

        model_type: "simple" (Prime Power only), "enhanced" (all MODEL_WEIGHTS features)
        """
        ...
        # Fix 3: composite score driven by MODEL_WEIGHTS dict
    ...
    def _normalize_features(self, df):
        ...
        Fix 2: Uses _minmax_scale() instead of x/max(), which correctly handles
        flat distributions (all values equal) on a per-row basis and avoids
        inflating every horse's score when there is no variance in a feature.
        """
    ...
    def print_detailed_predictions(self, race_num):
        ...
        feature_cols = ['Horse', 'ML_Odds', 'Market_Prob', 'Win_Prob'] + \
                       list(self.MODEL_WEIGHTS.keys()) + ['Composite_Score']
        available_cols = [col for col in feature_cols if col in df.columns]
        detail_df = df[available_cols].copy()

        weight_summary = ", ".join(
            f"{col.replace('_Norm', '')}={w:.0%}" for col, w in self.MODEL_WEIGHTS.items()
        )
        ...
        for col, weight in self.MODEL_WEIGHTS.items():
            print(f"    {col}: {top[col]:.3f} × {weight} = {top[col] * weight:.3f}")
...
if __name__ == "__main__":
    ...
    engine = ParxRacingEngineV3()

    print("\n" + "="*50)
    print(" PARX RACING PREDICTIVE ENGINE v3.0 (Kiro)")
    print("="*50)
